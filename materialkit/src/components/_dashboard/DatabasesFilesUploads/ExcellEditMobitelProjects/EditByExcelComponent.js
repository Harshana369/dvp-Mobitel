...
function EditByExcelComponent() {
  ...
  const Formatter = () => {
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      if (items[i].Implementation_By) {
        if (items[i].Implementation_By === 'Mobitel Direct') {
          setSuccessMessage('Your Excell is ready to upload !');
        } else if (items[i].Implementation_By !== 'Mobitel Direct') {
          setDateValidationError('Implementation_By format error !');
          setvalidationErrorMessage(
            'Invalid Implementation_By input. Please fill with "Mobitel Direct" !'
          );
          setTimeout(() => {
            setvalidationErrorMessage('');
          }, 8000);
        ...
        setvalidationErrorMessage(
          'Implementation_By input is empty. Please fill with "Mobitel Direct" !'
        );
        setTimeout(() => {
          setvalidationErrorMessage('');
        }, 8000);
  ...
  const uplaodHandler = async (e) => {
    ...
    // }
      ...
      axiosInstance
        .put('/mobitelProjectsDatabasesExcell/ExcellEdit', newPost)
        .then((res) => {
...
export default EditByExcelComponent;
